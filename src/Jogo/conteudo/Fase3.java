...
public class Fase3 extends Fases implements ActionListener {
	...
	private List<Boss1> boss1;
	private boolean jogando;
	private int inimigos_1 = 15;
	private int boss_1 = 1;
	...
	public Fase3() {
		...
		ImageIcon ref = new ImageIcon(getClass().getResource("/cont/Background3.png"));
		fundo = ref.getImage();

		iniciarSomFase("/cont/somfase3.wav");
		...
		bossTimer = new Timer(3000, new ActionListener() {

			@Override
			public void actionPerformed(ActionEvent e) {
				for (Boss1 boss : boss1) {
					if (boss.isVisivel()) {
						boss.bullets2();
					}
				}
		...
		if (inimigos.size() == 0 && !boss1.isEmpty()) {
	...
	public void iniciarBoss1() {
		int[] coorden = new int[boss_1];
		boss1 = new ArrayList<>();

		for (int i = 0; i < coorden.length; i++) {
			int x = 1048 + (int) (Math.random() * (1058 - 1048 + 1));
			int y = 90 + (int) (Math.random() * (600 - 90 + 1));
			boss1.add(new Boss1(x, y));
		}
	}
	...
	private void pintarRect(Graphics e) {
		e.setColor(Color.RED);

		// Desenha o bounds da nave
		Rectangle boundsNave = jogador.getBounds();
		e.drawRect(boundsNave.x, boundsNave.y, boundsNave.width, boundsNave.height);

		// Desenha o bounds de cada tiro do jogador
		for (Tiros t : jogador.getTiros()) {
			Rectangle boundsTiro = t.getBounds();
			e.drawRect(boundsTiro.x, boundsTiro.y, boundsTiro.width, boundsTiro.height);
		}

		// Desenha o bounds de cada boss e seus projéteis
		for (Boss1 boss : boss1) {
			Rectangle boundsBoss = boss.getBounds();
			e.drawRect(boundsBoss.x, boundsBoss.y, boundsBoss.width, boundsBoss.height);
			for (TiroBs1 tiro : boss.getTiroBs1()) {
				Rectangle boundsTiroBs1 = tiro.getBounds();
				e.drawRect(boundsTiroBs1.x, boundsTiroBs1.y, boundsTiroBs1.width, boundsTiroBs1.height);
			}
		}
	}
	...
	public void colisoes() {
		...
		Rectangle formaBsNave;
		Rectangle formaTiro;
		Rectangle formaTiroBs;
		...
		for (int i = 0; i < boss1.size(); i++) {
			vidaAnterior = jogador.getVIDAS();
			Boss1 tempboss = boss1.get(i);
			formaBsNave = tempboss.getBounds();

			if (formaNave.intersects(formaBsNave)) {
		...
		for (int i = 0; i < tiros.size(); i++) {
			...
			for (int j = 0; j < boss1.size(); j++) {
				...
				Boss1 tempboss = boss1.get(j);
				formaBsNave = tempboss.getBounds();

				if (formaTiro.intersects(formaBsNave)) {

					tempTiro.seteVisivel(false);

					boss1.get(j).setVIDAS(boss1.get(j).getVIDAS() - 1);

					if (boss1.get(j).getVIDAS() <= 0) {

						boss1.get(j).setVisivel(false);
		...
		List<TiroBs1> tirosInimigos;

		for (Boss1 inimigo : boss1) {

			vidaAnterior = jogador.getVIDAS();

			tirosInimigos = inimigo.getTiroBs1();
			for (int j = 0; j < tirosInimigos.size(); j++) {
				TiroBs1 tiroIn = tirosInimigos.get(j);
				formaTiroBs = tiroIn.getBounds();

				if (formaNave.intersects(formaTiroBs)) {
					if (jogador.isTurbo()) {

						tiroIn.setVisivel(false);
					} else {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (jogador != null && jogando) {
			...
			if (boss1.isEmpty() && jogador.getX() <= 1024 && jogador != null) {
		...
		for (int i = 0; i < boss1.size(); i++) {
			...
			if (t.isVisivel()) {
				...
				// Atualiza os tiros do inimigo
				List<TiroBs1> tirosIn = t.getTiroBs1();
				for (int j = 0; j < tirosIn.size(); j++) {
					TiroBs1 tiro = tirosIn.get(j);
					if (tiro.isVisivel()) {
						tiro.update();
					} else {
						tirosIn.remove(j);
		...
		if (inimigos.size() != 0 && !boss1.isEmpty()) {
			boss1.get(0).setVELOCIDADE(0);
		} else {

			if (!boss1.isEmpty()) {

				if (boss1.get(0).getVIDAS() > 3) {
					boss1.get(0).setVELOCIDADE(5);
					if (!bossTimer.isRunning()) {
				...
				if (boss1.get(0).getVIDAS() <= 3) {
					boss1.get(0).setVELOCIDADE(8);
					bossTimer.setDelay(500);
				}
